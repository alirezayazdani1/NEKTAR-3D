...
#define  SEC_TIME     sections[11]             /* Time Dependence info       */
...
void ReadParams (FILE *fp)
{
  ...
  if(!iparam("MODES")        ) iparam_set("MODES",iparam("NORDER"));
	...
	// Alireza: setting the number of chemical components
#ifdef ADR
	if (!iparam("NSPEC")){
		ROOTONLY fprintf(stdout, "Number of species not defined: nspecs = 1\n");
		nspecs = 1;
		iparam_set("NSPEC", nspecs);
	} else nspecs = iparam("NSPEC");
#endif
...
void ReadPscals (FILE *fp, Domain *omega)
{
  int  n;
  char buf[BUFSIZ];
...
#ifdef ADR
	omega->Pr	= dvector(0,nspecs-1);
	for (int i = 0; i < nspecs; i++) omega->Pr[i] = tmp[i];
#endif
...
Element_List *ReadMesh (FILE *fp, char* session_name)
{
  ...
  // geometric factors are set in part_elmt_list
...
static void ReadCurve(FILE *fp, Element_List *new_E){
  ...
  if(strstr(buf,"type")){
	...
	else if(strstr(buf,"Tau")||strstr(buf,"tau")){ /* Taurus ?? */
	  fgets (buf, BUFSIZ, fp);
	  sscanf (buf, "%lf%lf%lf%lf%lf%lf%lf%lf%1s",
		  &(ctmp[i].info.tau.xc), &(ctmp[i].info.tau.yc),
		  &(ctmp[i].info.tau.zc), &(ctmp[i].info.tau.ax),
		  &(ctmp[i].info.tau.ay), &(ctmp[i].info.tau.az),
		  &(ctmp[i].info.tau.axialradius),
		  &(ctmp[i].info.tau.piperadius),tagid+i);

	  /* normalise ax,ay,az */
	  fac = sqrt(ctmp[i].info.tau.ax*ctmp[i].info.tau.ax + 
		     ctmp[i].info.tau.ay*ctmp[i].info.tau.ay + 
		     ctmp[i].info.tau.az*ctmp[i].info.tau.az);
	  ctmp[i].info.tau.ax /= fac;
	  ctmp[i].info.tau.ay /= fac;
	  ctmp[i].info.tau.az /= fac;
	  ctmp[i].type = T_Taurus;
	}
	...
	for(i = 0; i < k; ++i){
	  ...
	  if (fscanf(fp, "%lf%lf%1s", &fac, &d_eid, &type) != 3)
	...
	case 'S':                                   /* Straight side */
	  curve[0].type = T_Straight;
	  break;
	case 'C':                                   /* Circular arc */
	  sscanf (buf, "%*d%*d%lf",&(curve[0].info.arc.radius));
	  curve[0].type = T_Arc;
	  break;
	default: 
...
void ReadICs (FILE *fp, Domain *omega)
{
  ...
  case 3: {                          /* Function(x,y,z) Initial conditions */
    ...
    if(omega->Tfun && omega->Tfun->type == Womersley){
      fprintf(stdout,"Setting up initial field as exact womersley solution"
	      " at t = %lf \n",dparam("STARTIME"));
      SetWomField(omega,U[0]->base_h,U[1]->base_h,
		  U[2]->base_h,dparam("STARTIME"));
      for (i = 0; i < nfields+1; i++)
	set_state(U[i]->fhead,'p');
    }
    else{
...
void ReadDF(FILE *fp, Domain *omega)
{
  char buf[BUFSIZ], *s;
  int  i, nlines;
  int nfields = omega->U->fhead->dim();
#ifdef ADR
	nfields += nspecs;
#endif
  rewind(fp);

  omega->sources = (char**) malloc(nfields*sizeof(char*));
...
if (nlines != 0){
    for (i = 0; i < nfields; ++i){
      if(s = strchr(fgets(buf, BUFSIZ, fp),'=')){
  while (isspace(*++s));
  omega->sources[i] = (char*) malloc(BUFSIZ*sizeof(char));
  strcpy(omega->sources[i], s);
      }
      else{
  omega->sources[i] = (char*) malloc(BUFSIZ*sizeof(char));
  sprintf(omega->sources[i], "null");
      }
    }
}
else
...
Bndry *ReadMeshBCs (FILE *fp, Element_List *Mesh){
  ...
  char       szTemp[BUFSIZ]; /* to save pressure BC string here */
  ...
  for(E=Mesh->fhead;E;E=E->next){ 
    ...
    for (n = 0; n < ntot; ++n){
      ...
      /*Alireza:  read label of boundary if exist */
      szTemp[0] = '\0'; /* clear string */
      sscanf(p, "%d%d%lf%lf%lf%s", &iel, &iside,f,f+1,f+2,szTemp); 
	...
	new_bndry->face = n;
	new_bndry->elmt = E;
	new_bndry->bvert = dvector(0,0);
	new_bndry->bvert[0] = -1;
	/* store face with dirichlet condition in bvert storage */
	if(f[0]){
	  new_bndry->type = 'V';
	  new_bndry->bvert[0] = 0;
	  E->set_solve(n,off);
	  option_set("REFLECT0", 1);
	}
	else if(f[1]){
	  new_bndry->type = 'F';
	  new_bndry->bvert[0] = 1;
	  option_set("REFLECT1", 1);
	}
	else if(f[2]){
	  new_bndry->type = 'F';
	  new_bndry->bvert[0] = 2;
	  option_set("REFLECT2", 1);
	}
	else
	  fprintf(stderr,"Error: Symmetry boundary in elmt %d, face %d"
		  " does not have a direction prescribed\n",iel,iside);
	break;
      case 'W':  case 'V':  case 't': case 'v': 
	E->set_solve(n,off);
	new_bndry = (Bndry *)calloc(1,sizeof(Bndry));
	...
	new_bndry->elmt = E;
       ...
       /* Alireza -- new part: required to label inlets */
        if ( bc == 'v' || bc == 'V' ){
           new_bndry->blabel = (char*) calloc(BUFSIZ,sizeof(char)); //Alireza: save label for inlet BC
           if(szTemp[0] == '\0')
               sprintf(new_bndry->blabel,"0.0\n");
           else
               sprintf(new_bndry->blabel,"%s\n",szTemp);
        }
	break;
	...
	new_bndry->elmt = E;
       ...
       /* Alireza -- new part: required to label outlets */
       /* save pressure Dirichelet BC in new_bndry->bstring */
        new_bndry->bstring = (char*) calloc(BUFSIZ, sizeof(char));
        new_bndry->blabel = (char*) calloc(BUFSIZ,sizeof(char));	//Alireza: save label for outlet BC
        if(szTemp[0] == '\0') {
          sprintf(new_bndry->bstring,"0.0\n");
          sprintf(new_bndry->blabel,"0.0\n");
        }
        else{
          sprintf(new_bndry->bstring,"%s\n",szTemp);
          sprintf(new_bndry->blabel,"%s\n",szTemp);
        }
	break;
      default:
  ...
  return bsort(bndry_list,nr_bcs);      /* Return list */
}
	...
	*new_bndry  = (Bndry *) NULL;
	...
	for(E=Mesh->fhead;E;E=E->next){ 
		...
		for (n = 0; n < ntot; ++n){
			...
			/* vertex solve mask is on by default */
			while (isspace(*p)) p++; bc = *p++;
      sscanf(p, "%d%d%n", &iel, &iside, &csize);
      p += csize;
      for (i = 0; i < nspecs; ++i){
        sscanf(p, "%lf%n", f+i, &csize);
        p += csize;
      }
				...
				case 'Z': 
					...
					if(f[0]){
						...
						fprintf(stderr,"Error: Symmetry boundary in elmt %d, face %d"
								" does not have a direction prescribed\n",iel,iside);
					break;
				 case 'W': case 'v': case 'V': case 't':	// Alireza: 'W' zero-Dirichlet
					E->set_solve(n,off);
					new_bndry = (Bndry *)calloc(1,sizeof(Bndry));
					new_bndry->type = toupper(bc);
					new_bndry->usrtype = bc;
...
Bndry *ReadBCs (FILE *fp, Element *U)
{
  register int   n,i;
  char       bc, buf[BUFSIZ], *p;
  int        iel, iside, nbcs=0,ntot,k,nfields,csize;
  ...
  char       szTemp[BUFSIZ];   /* to save pressure BC string         */
 ...
 /* ....... Begin Reading Boundary Conditions ...... */
  E = U;
  for(k =0; (k < pllinfo.gnel) && (E); ++k){
    
    sscanf(p, "%*1s%d%d", &iel, &iside); 
    
    if(iel - pllinfo.eloop[E->id] != 1){ /* skip to next element */
      
      ntot = (U->dim() == 3) ? pllinfo.efaces[k] : pllinfo.eedges[k];
  ...
  for (i = 0; i < E->dim(); ++i){
	...
	if ( bc == 'V' || bc == 'v' || bc == 'O' || bc == 'o' ){
#ifdef ADR
		if (!bcADR){ sscanf(p, "%s%n", szTemp, &csize); p += csize; }
#else
		sscanf(p, "%s%n", szTemp, &csize);
		p += csize;
#endif
	}
	
	if (iel - pllinfo.eloop[E->id] != 1 || iside - n != 1) {
	  sprintf(buf, "Mismatched element/side -- got %d,%d, expected %d,%d",
		  iel, iside, pllinfo.eloop[E->id]+1, n+1);
	  fprintf(stderr,"ReadBCs : %s\n", buf); exit(-1);
	}
	switch (bc) {
				
		case 'Z': /* symmetry boundary condition */
			/* set dirichlet condition depending on definition of f[fnum] */
			if(f[fnum]){ 
				E->set_solve(n,off);
				new_bndry = E->gen_bndry('V', n, 0.0);
			}
			else
		...
		case 'V':
			...
			new_bndry->usrtype = bc;
          if (bc == 'V'){
             new_bndry->blabel = (char*) calloc(BUFSIZ,sizeof(char)); /* Alireza: allocate memory for label */
             sprintf(new_bndry->blabel,"%s\n",szTemp);  /* Alireza: fill blabel */
          }
			break;
		case 'I': case 'O': 
			new_bndry = E->gen_bndry(bc, n, 0.0);
			/* save f[fnum] for specifying outflow pressure value */	  
			sprintf(buf,"%lf",f[fnum]);
			new_bndry->bstring = strdup(buf);
			new_bndry->usrtype = bc;
          if (bc == 'O'){
           new_bndry->blabel = (char*) calloc(BUFSIZ,sizeof(char)); /* Alireza: allocate memory for label */
           sprintf(new_bndry->blabel,"%s\n",szTemp);  /* Alireza: fill blabel */
...
#ifdef ADR
	if (fnum == 3 && !bcADR) {fnum = 0; bcADR = 1;}
#endif
  ...
  if (omega->his_list = his_list) {
	...
	if(hp->mode == TransVert)
	  printf ("%2d: Proc %d, vertex = %2d, elmt = %3d, fields = %s\n",
		  ++cnt,pllinfo.procid, hp->i + 1, pllinfo.eloop[hp->id] + 1,
		  hp->flags);
...
void ReadSoln(FILE* fp, Domain* omega){
  ...
  if(findSection("Exact", buf, fp)||findSection("EXACT", buf, fp)){
    for(i=0;i<nfields;++i){
      if(s = strchr(fgets(buf,BUFSIZ,fp),'=')){
	while (isspace(*++s));
    ...
    ROOTONLY{
      ...
      fprintf(stdout,"Exact W : %s",omega->soln[2]);
...
void ReadTimeDepend(FILE *fp, Domain *omega){
  ...
  if(findSection(SEC_TIME,buf,fp)){
    ...
    omega->Tfun = (Tfunct *)calloc(1,sizeof(Tfunct));
    option_set("tvarying",2);

    fgets(buf,BUFSIZ,fp);
    if(strstr(buf,"Str")||strstr(buf,"str")){
      omega->Tfun->type = String;

      for(i = 0; i < n; ++i){
	p = fgets(buf,BUFSIZ,fp);
	if((p = strchr(p,'=')) == (char *) NULL)
	  error_msg(ReadTimeDepend: have not defined all time functions);
	omega->Tfun->info.string[i].TimeDepend = strdup(++p);
      }
      for(i = n; i < dim; ++i)
	omega->Tfun->info.string[i].TimeDepend = (char *) 0;
    }
    else if(strstr(buf,"Fou")||strstr(buf,"fou")){
      int j;
      int nmod;
      omega->Tfun->type = Fourier;
      
      for(i = 0; i < n; ++i){
	fgets(buf,BUFSIZ,fp);
	sscanf(buf,"%d",&nmod);
	omega->Tfun->info.fexpand[i].nmodes = nmod;
	fgets(buf,BUFSIZ,fp);
	sscanf(buf,"%lf",&omega->Tfun->info.fexpand[i].wnum);
	omega->Tfun->info.fexpand[i].ccos = dvector(0,nmod-1);
	omega->Tfun->info.fexpand[i].csin = dvector(0,nmod-1);

	for(j = 0; j < nmod; ++j){
	  fgets(buf,BUFSIZ,fp);
	  sscanf(buf,"%lf%lf",omega->Tfun->info.fexpand[i].ccos + j,
		 omega->Tfun->info.fexpand[i].csin + j);
	  
	}
      }
    }
    else if(strstr(buf,"Wom")||strstr(buf,"wom")){
      int j;
      int nmod;
      
      omega->Tfun->type = Womersley;
      option_set("TIMEVBCS",2);

      fgets(buf,BUFSIZ,fp);
      if(strstr(buf,"Enc")||strstr(buf,"enc")){
             omega->Tfun->info.wexpand.type = ENCODED;
      } else if(strstr(buf,"Raw")||strstr(buf,"raw")) {
             if (!iparam("NFOU"))iparam_set("NFOU",24);
             omega->Tfun->info.wexpand.type = RAW;
      } else {
             fprintf(stderr,"Unknown symbol in womersley data\n");
             exit(-1);
      }
      fgets(buf,BUFSIZ,fp);
      if(strstr(buf,"Pre")||strstr(buf,"pre"))
             omega->Tfun->info.wexpand.form = PRESSURE;
      else if(strstr(buf,"Mas")||strstr(buf,"mas"))
             omega->Tfun->info.wexpand.form = MASSFLOW;
      else {
             fprintf(stderr,"Unknown symbol in womersley data\n");
             exit(-1);
      }
      ...
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf",&omega->Tfun->info.wexpand.scal0);

      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf",&omega->Tfun->info.wexpand.radius);
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf%lf%lf",omega->Tfun->info.wexpand.axispt,
	     omega->Tfun->info.wexpand.axispt+1,
	     omega->Tfun->info.wexpand.axispt+2);
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf%lf%lf",omega->Tfun->info.wexpand.axisnm,
	     omega->Tfun->info.wexpand.axisnm+1,
	     omega->Tfun->info.wexpand.axisnm+2);
      
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%d",&nmod);
      if (omega->Tfun->info.wexpand.type == ENCODED)
          omega->Tfun->info.wexpand.nmodes = nmod;
      else
          omega->Tfun->info.wexpand.nraw = nmod;

      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf",&omega->Tfun->info.wexpand.wnum);

      if (omega->Tfun->info.wexpand.type == ENCODED) {
        omega->Tfun->info.wexpand.ccos = dvector(0,nmod-1);
        omega->Tfun->info.wexpand.csin = dvector(0,nmod-1);
        for(j = 0; j < nmod; ++j){
	  fgets(buf,BUFSIZ,fp);
	  sscanf(buf,"%lf%lf",omega->Tfun->info.wexpand.ccos + j,
	       omega->Tfun->info.wexpand.csin + j);
        }
      } else {
        omega->Tfun->info.wexpand.raw = dvector(0,nmod-1);
        for(j = 0; j < nmod; ++j){
	  fgets(buf,BUFSIZ,fp);
	  sscanf(buf,"%lf",omega->Tfun->info.wexpand.raw + j);
        }
      }
