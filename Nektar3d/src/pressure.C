...
void SetPBCs(Domain *omega){
  ...
  N[0] =  omega->Uf, N[1] = omega->Vf, N[2] = omega->Wf;
   ...
   if  ( step % iparam("HISSTEP") == 0 ){
     ...
     DO_PARALLEL
       gdsum(flowrate,(Nout+Ninl),work);
		 ...
		 for (i = 0; i < Nout; ++i){
       dtemp = Flux(omega, omega->IMPBC[0].standard_labels[i]);
       if (Communicator_outlet[i] != MPI_COMM_NULL)
         MPI_Allreduce(&dtemp,&flowrate[i],1,MPI_DOUBLE,MPI_SUM,Communicator_outlet[i]);
     }
   }
   ...
   if (omega->IMPBC[0].Nimpedance_modes > 0)
      ...
      omega->IMPBC[0].ComputePressureSteady(flowrate);
