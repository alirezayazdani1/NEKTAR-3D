...
//  gmap_Temp  = GlobalNumScheme (Mesh, MeshbcTemp);
  ...
  gmap_Temp				= GlobalNumScheme(Mesh, MeshbcTemp);
	omega->T[0]			= omega->U->gen_aux_field('t');
	Replace_Numbering(omega->T[0], Mesh);
  if(slvtype == SubStep) set_elmt_sides(omega->T[0]);
  omega->Tbc[0]		= ReadBCs(adr_file, omega->T[0]->fhead);   
  omega->Tsys[0]	= gen_bsystem(omega->T[0], gmap_Temp);
  free_gmap(gmap_Temp);
