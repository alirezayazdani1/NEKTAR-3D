...
class Bio_Model{
 ...
 public:
  ...
  virtual void Particle_Wall_Interaction(const int pid, const int bid)
    //Called if particle interacts with the wall
    {fprintf(stderr,"Virt func is being called!!\n"); return;};
#ifdef APSOPT
  virtual void Particle_Wall_Interaction(const int pid, const double px, const double py, const double pz)
    //Called if particle interacts with the wall
    {fprintf(stderr,"Virt func is being called!!\n"); return;};
#endif
...
#ifdef APSOPT
  void Particle_Wall_Interaction(const int pid, const double px, const double py, const double pz){return;};
#endif
...
#ifdef APSOPT
  void Particle_Wall_Interaction(const int pid, const double px, const double py, const double pz);
#endif
...
class Particle{
 ...
 public:
  ...
  int Particle_Particle_Dissipation(FCM_Domain* FCM_Omega, int pid2,
                              double &dx, double &dy, double &dz,
                              double &fm){return -1;};
  virtual void Wall_Force(FCM_Domain* FCM_Omega, 
			  double &f0, double &f1, double &f2){return;};
};//class Particle
...
class Sphere : public Particle{
 ...
 public:
  ...
  int Particle_Particle_Dissipation(FCM_Domain* FCM_Omega, int pid2,
                              double &dx, double &dy, double &dz,
                              double &fm);
  void Wall_Force(FCM_Domain* FCM_Omega, double &f0, double &f1, double &f2);
};//class Sphere
...
class FCM_Domain{
 ...
 public:
...
#ifdef FBAR
  Dvector FBAR_list;//temp store for parallel runs
#endif
#endif //PARALLEL
  ...
  void Write_Particle_Field(FILE* out,int step,double time);
