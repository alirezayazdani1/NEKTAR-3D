...
void AddCCSource(Domain *);
void CalcTbc(Bndry *B, Element_List **V, double Pec, int index);

const double tscale = 0.005;				// problem time scale (sec)
const double lscale = 1.0e-3;			// problem length scale (m)

const double mu = 3.0e-2/tscale, stddev = sqrt(0.2)/tscale;		// agonist release Gaussian parameters (5 sec release time constant of ADP)
...
const double relCon[] = { 0.0, 170.0/(stddev*sqrt(2.0*M_PI)) };		// release content (nmol)

// setting the reaction rates ( 1/sec, 1/(nMsec), nM )
const double k9 = (11.0/60.0)*tscale*1.0e3, K9M = 160.0, h9 = (0.0162/60.0)*tscale*1.0e3, k8 = (194.4/60.0)*tscale*1.0e3, K8M = 1.12e5, h8 = (0.222/60.0)*tscale*1.0e3, 
					hC8 = (10.2/60.0)*tscale*1.0e3, HC8M = 14.6, k5 = (27.0/60.0)*tscale*1.0e3, K5M = 140.5, h5 = (0.17/60.0)*tscale*1.0e3, hC5 = (10.2/60.0)*tscale*1.0e3, 
					HC5M = 14.6, k10 = (2391.0/60.0)*tscale*1.0e3, K10M = 160.0, h10 = (0.347/60.0)*tscale*1.0e3, hTFPI = (0.48/60.0)*tscale*1.0e3, k2 = (1344.0/60.0)*tscale*1.0e3, 
					K2M = 1060.0, h2 = (0.714/60.0)*tscale*1.0e3, k1 = (3540.0/60.0)*tscale*1.0e3, K1M = 3160.0, k11 = (0.0078/60.0)*tscale*1.0e3, K11M = 50.0,
        	h11A3 = (1.6e-3/60.0)*tscale*1.0e3, h11L1 = (1.3e-5/60.0)*tscale*1.0e3, kPC = (39.0/60.0)*tscale*1.0e3, KPCM = 3190.0, hPC = (6.6e-7/60.0)*tscale*1.0e3, 
					KdZ = 0.56, KdW = 0.1, k79 = (32.4/60.0)*tscale*1.0e3, K79M = 24.0, k710 = (103.0/60.0)*tscale*1.0e3, K710M = 240.0;
#endif
...
const double relCon[] = {0.0, 250.0/(stddev*sqrt(2.0*M_PI)), 10.0/(stddev*sqrt(2.0*M_PI))};		// release content (nmol)

const double kadh = 2.0e2*tscale, kcohxPLTmax = 1.0e4*tscale, kadp = 0.34*tscale, kiia = 0.5*tscale, ktxa2 = 1.0*tscale, k1 = (2.82*1.0e3/60.0)*tscale, ksurf = 5.0e-6*tscale, kiiAP = 1.0*tscale, kin = 0.025*tscale; // TODO: all physical units; must be converted !!!
